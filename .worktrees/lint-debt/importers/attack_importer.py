...
def parse_attack_stix(
    json_path: str,
) -> tuple[list[ATTACKObject], list[ATTACKRelationship]]:
...
def import_attack_to_neo4j(
    driver,
    objects: list[ATTACKObject],
    ...
    with driver.session() as session:
        for rel_type, neo4j_type in rel_map.items():
            typed_rels = [
                ...
                and r.source_ref in valid_ids
                and r.target_ref in valid_ids
            ]
            if not typed_rels:
                continue
