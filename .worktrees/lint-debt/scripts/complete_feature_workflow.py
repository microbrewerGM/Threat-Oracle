...
def wait_for_ci_checks(pr_number):
    ...
    for attempt in range(max_attempts):
        ...
        if attempt < max_attempts - 1:
            print(
                f"CI checks still running. Waiting 30 seconds... ({attempt + 1}/{max_attempts})"
            )
            time.sleep(30)
...
def approve_pull_request(pr_number):
    ...
    try:
        ...
        if pr_author == current_user:
            ...
            print(
                f"PR URL: https://github.com/$(gh repo view --json owner,name -q '.owner.login+\"/\"+.name')/pull/{pr_number}"
            )
...
def merge_pull_request(pr_number):
    ...
    try:
        ...
        if not mergeable:
            print(
                "PR is not mergeable. There might be conflicts that need to be resolved."
            )
            return False

        if review_decision == "CHANGES_REQUESTED":
            print(
                "Changes have been requested on this PR. Please address the feedback before merging."
            )
            return False
    ...
    if result.returncode != 0:
        ...
        print(
            "This could be due to branch protection rules requiring approvals or passing CI checks."
        )

        # Ask if the user wants to open the PR in the browser
        response = input(
            "Do you want to open the PR in your browser to check the status? (y/n): "
        )
        if response.lower() == "y":
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Automate the PR creation, review, merge, and cleanup process."
    )
    parser.add_argument("--title", help="Pull request title")
    parser.add_argument("--body", help="Pull request body")
    parser.add_argument(
        "--skip-ci", action="store_true", help="Skip waiting for CI checks"
    )
    ...
    print(
        "The feature has been merged into main and the local branch has been cleaned up."
    )
    return 0
